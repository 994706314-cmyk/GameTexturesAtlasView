...
class SettingsDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        changelog_text = QLabel(
            ...
            "  · 项目上传至 GitHub，支持通过 Releases 分发更新\n\n"
            "V1.4（2026-03-10）\n"
            "  · 检查模式：导出报告改为异步后台线程，底部进度条显示导出进度\n"
            "  · 检查模式：图集列表支持多选（Ctrl/Shift）+ 右键菜单\n"
            "  · 检查模式：导入文件/文件夹改为增量模式，不再清空已有列表\n"
            "  · 检查模式：新增独立文件菜单（新建/打开/保存/另存为/最近打开/导出报告）\n"
            "  · 检查模式：独立存档格式（.tcheck），与规划模式互不干扰\n"
            "  · 规划模式：修复「等比」缩放使用原图比例而非规划尺寸比例的问题\n"
            "  · UI：修复检查模式下菜单栏「文件」和「设置」顺序颠倒的问题\n"
            "  · 设置-关于：新增更新日志\n\n"
            "V1.3（2026-03-09）\n"
            "  · 新增截图贴图功能（Alt+D 全局热键），快速添加占位贴图\n"
            "  · 规划模式 Excel 导出改为双模式（预览/完整）异步导出\n"
            "  · 修复 QPixmap.scaled 参数错误导致 exe 启动崩溃的问题\n"
            "  · 应用图标更新（透明背景 RGBA + 多尺寸 ICO）\n\n"
            "V1.2\n"
            "  · 检查模式：梯次网格扫描 + 跨图集重复检测\n"
            "  · 检查模式：彩色边框可视化标记 + 分析报告导出\n"
            "  · 新增最低检测档位设置（默认 64×64）\n"
            "  · 底部工具栏新增分批渲染进度条\n\n"
            "V1.1\n"
            "  · 自动映射压缩：导入时按宽度映射表自动设置规划尺寸\n"
            "  · 宽度配色区分：素材库底色按宽度分类着色\n"
            "  · 撤销/重做系统（可配置步数上限）\n"
            "  · 自定义快捷键 + 导入排除后缀\n\n"
            "V1.0\n"
            "  · 初始版本：拖拽排列、多合图管理、Excel 导出\n"
            "  · 项目保存/打开（.tatlas 格式）"
        )
