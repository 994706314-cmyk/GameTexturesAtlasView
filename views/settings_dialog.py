...
class SettingsDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        changelog_text = QLabel(
            f"V{APP_VERSION}（2026-04-24）\n"
            "  · 修复：刷新素材库时缺失源文件的贴图不再丢失缩略图，改为保留并显示「缺失」小角标\n"
            "  · 改进：「用新文件重新导入」默认打开当前图片所在目录\n"
            "  · 改进：调整合图尺寸时自动重新排布贴图，放不下才提醒（不再简单拒绝）\n"
            "  · 提示：画布中选中贴图后按 Delete 键可从合图中移除（不影响素材库）\n"
            "  · 新增：拖入 .tatlas/.tcheck 存档自动识别并切换到对应模式打开\n"
            "  · 新增：双击存档文件直接启动工具并打开（需先在设置中注册文件关联）\n"
            "  · 新增：设置菜单「注册文件关联」功能，为存档文件绑定图标和打开方式\n"
            "  · 改进：画布中左键单击选中贴图（不再立即拖拽），拖动超过阈值才移动\n\n"
            "V1.9.8（2026-04-24）\n"
            "  · 新增：右键菜单「用新文件重新导入」功能，支持替换贴图源文件并保留软件内名称\n"
            "  · 新增：文件菜单「查找缺失文件」功能，按同名文件批量恢复丢失的图片链接\n"
            "  · 改进：「设置分辨率」改为子菜单，新增 7 种快捷分辨率预设\n"
            "  · 新增：画质类型角标（High/VeryHigh/Extreme），支持批量设置\n\n"
            "V1.9.7（2026-04-22）\n"
            "  · 修复：打包后缺少 unittest/pydoc 模块导致明度查重功能无法使用的问题"
        )
