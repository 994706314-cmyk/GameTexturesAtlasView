...
class SettingsDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        changelog_text = QLabel(
            ...
            "  · 追加支持轻量存档（仅缩略图）和全量存档（含原图），格式统一\n\n"
            "V1.9.3（2026-03-12）\n"
            "  · 规划模式：素材库列表视图合图列显示标记角标和图标样式序号\n"
            "  · 规划模式：合图序号采用蓝色圆角矩形图标样式\n\n"
            "V1.9.2（2026-03-12）\n"
            "  · 规划模式：图集卡片角标优化，序号和标记上下排列更整齐\n"
            "  · 规划模式：素材库新增按标记种类排序、按所在图集排序\n"
            "  · 规划模式：缩短搜索框宽度，排序按钮布局更紧凑\n\n"
            "V1.9.1（2026-03-12）\n"
            "  · 规划模式：标记类型新增 M（Mask），自动规划合图分组加入 M\n"
            "  · 规划模式：图集卡片显示标记类型角标（E/A/M/C1/C2/C3）\n"
            "  · 规划模式：编辑界面贴图右上角显示标记类型角标\n\n"
            "V1.9.0（2026-03-12）\n"
